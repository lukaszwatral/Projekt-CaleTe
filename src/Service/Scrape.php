...
class ScrapeData
{
    ...
    public function scrapeData(string $department){
        $apiURL = "https://plan.zut.edu.pl/schedule_student.php?kind=apiwi&department={$department}&start=2024-10-7&end=2024-10-31";
    ...
    {
        $stmt = $this->pdo->prepare("SELECT id FROM Tok_studiow WHERE  typ = :typ AND typ_sk = :typ_sk AND tryb = :tryb AND tryb_sk = :tryb_sk");
    ...
    {
        $stmt = $this->pdo->prepare("SELECT id FROM Tok_studiow WHERE tryb_sk = :tryb AND typ_sk = :typ");
        $stmt->bindParam(':tryb', $tryb);
        $stmt->bindParam(':typ', $typ);
        $stmt->execute();
        $result = $stmt->fetch();
        ...
        if($result){
            $tok_id = $result['id'];
            $stmt = $this->pdo->prepare("SELECT id FROM Przedmiot WHERE nazwa = :przedmiot AND tok_id = :tok_id");
            $stmt->bindParam(':przedmiot', $przedmiot);
            $stmt->bindParam(':tok_id', $tok_id);
            $stmt->execute();
            $result = $stmt->fetch();
            if(!$result){
                $stmt = $this->pdo->prepare("INSERT INTO Przedmiot (nazwa, tok_id) VALUES (:przedmiot, :tok_id)");
                $stmt->bindParam(':przedmiot', $przedmiot);
                $stmt->bindParam(':tok_id', $tok_id);
                $stmt->execute();
            }
    ...
    private function insertZajecia(int $id, string $data_start, string $data_koniec, string $zastepca, string $wykladowca, string $wydzial, string $grupa, string $tok_studiow)
    {
        ...
        if($result){
            ...
            if($result){
                ...
                if($result) {
                    $grupa_id = $result['id'];
                    $stmt = $this->pdo->prepare("SELECT id FROM Tok_studiow WHERE typ_sk = :tok_studiow");
                    ...
                    if ($result) {
                        ...
                        $stmt->bindParam(':tok', $tok_studiow_id);
                        $stmt->execute();
                    }
                }
            }
